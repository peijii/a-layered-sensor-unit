...
def train_xgb(X, y, show_confusion=False):
    #le = LabelEncoder()
    ...
    for train_index, test_index in kf.split(X):
        ...
        best_xgbc = gridsearch.best_estimator_
        print("Best parameters")
        print(gridsearch.best_params_)
        acc = best_xgbc.score(Xtest, Ytest)

        if show_confusion:
            conf_mx = confusion_matrix(Ytest, best_xgbc.predict(Xtest))
            plot_confusion_matrix(conf_mx, classes=classes, normalize=True,
                          title='Confusion matrix, with normalization')

        res = classification_report(Ytest, best_xgbc.predict(Xtest), output_dict=True)
        # print(res)
